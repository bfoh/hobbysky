...
export async function sendBookingConfirmation(
  ...
  try {
    ...
    const htmlContent = generateEmailHtml({
      ...
      preheader: `Reservation confirmed for ${guest.name} at Hobbysky Guest House`,
      content: `
        <p>Dear <strong>${guest.name}</strong>,</p>
        <p>Thank you for choosing hobbysky guest house. Your reservation has been successfully confirmed. We look forward to hosting you!</p>
    ...
    const emailPayload: any = {
      to: guest.email,
      subject: 'Booking Confirmed - Hobbysky Guest House',
      html: htmlContent,
      text: `
Booking Confirmed - hobbysky guest house

Dear ${guest.name},

Thank you for choosing hobbysky guest house. Your reservation has been successfully confirmed.
...
export async function sendCheckInNotification(
  ...
  try {
    ...
    const htmlContent = generateEmailHtml({
      title: 'Welcome to Hobbysky Guest House',
      preheader: `Check-in confirmed for Room ${room.roomNumber}`,
      content: `
        <p>Dear <strong>${guest.name}</strong>,</p>
        <p>Welcome to hobbysky guest house! Your check-in has been confirmed. We hope you have a wonderful stay with us.</p>
    ...
    const result = await sendTransactionalEmail({
      to: guest.email,
      subject: 'Welcome to Hobbysky Guest House - Check-In Confirmed',
      html: htmlContent,
      text: `
Welcome to hobbysky guest house!
...
export async function sendCheckOutNotification(
  ...
  try {
    ...
    const htmlContent = generateEmailHtml({
      ...
      content: `
        <p>Dear <strong>${guest.name}</strong>,</p>
        <p>Thank you for choosing hobbysky guest house! Your check-out has been successfully processed.</p>
    ...
    const result = await sendTransactionalEmail({
      to: guest.email,
      subject: 'Thank You for Staying at Hobbysky Guest House',
      html: htmlContent,
      text: `
Thank You for Staying at hobbysky guest house!

Dear ${guest.name},

Thank you for choosing hobbysky guest house! Your check-out has been processed.
    ...
    if (settings.managerEmail) {
      ...
      const htmlContent = generateEmailHtml({
        ...
        content: `
          ...
          <p style="color: #666; font-size: 14px;">This is an automated notification from the Hobbysky Guest House management system.</p>
        `
      })
  ...
  try {
    ...
    if (newGuest.email && !newGuest.email.includes('@guest.local')) {
      const guestHtml = generateEmailHtml({
        ...
        preheader: `Welcome to the group ${groupReference} at Hobbysky Guest House`,
        content: `
          <p>Dear <strong>${newGuest.name}</strong>,</p>
          <p>You have been added to a group booking at hobbysky guest house. Here are your reservation details:</p>
      ...
      await sendTransactionalEmail({
        to: newGuest.email,
        subject: `Group Booking Confirmation - ${groupReference} | Hobbysky Guest House`,
        html: guestHtml,
        text: `Welcome to hobbysky guest house!\n\nYou have been added to group booking ${groupReference}.\n\nRoom: ${room.roomNumber}\nCheck-in: ${checkInDate.toLocaleDateString()}\nCheck-out: ${checkOutDate.toLocaleDateString()}\n\nWe look forward to welcoming you!`
      })
    ...
    if (newGuest.phone) {
      ...
      const smsMessage = `hobbysky guest house: Hi ${newGuest.name}, you've been added to group booking ${groupReference}. Room ${room.roomNumber}, Check-in: ${checkInDate.toLocaleDateString()}. We look forward to hosting you!`
    ...
    if (billingContact && billingContact.email && billingContact.email !== newGuest.email) {
      ...
      await sendTransactionalEmail({
        ...
        subject: `Group Update: New Member Added - ${groupReference} | Hobbysky Guest House`,
        html: contactHtml,
    ...
    if (billingContact?.phone && billingContact.phone !== newGuest.phone) {
      const { sendSMS } = await import('@/services/sms-service')
      const smsMessage = `hobbysky guest house: ${newGuest.name} has been added to your group booking ${groupReference} (Room ${room.roomNumber}).`
...
export async function sendGroupMemberUpdatedNotification(
  ...
  try {
    ...
    await sendTransactionalEmail({
      to: guest.email,
      subject: `Booking Updated - ${groupReference} | Hobbysky Guest House`,
      html: htmlContent,
    ...
    if (guest.phone) {
      const { sendSMS } = await import('@/services/sms-service')
      const smsMessage = `hobbysky guest house: Your booking info for group ${groupReference} has been updated. ${changes.map(c => `${c.field}: ${c.newValue}`).join(', ')}.`
