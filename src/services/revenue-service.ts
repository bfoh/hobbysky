...
/**
 * Fetch all checked-in/checked-out bookings created by a specific staff member
...
export async function fetchBookingsForStaffWeek(
  ...
  for (const [bookingId, charges] of chargesByBookingId.entries()) {
    if (!allStaffBookingIds.has(bookingId)) continue
 ...
 * Always recalculates from live bookings so numbers stay up-to-date in real time.
 */
...
/**
 * Staff submits their weekly report.
 */
...
export async function getStaffAllReports(staffId: string): Promise<WeeklyRevenueReport[]> {
  ...
  try {
    ...
    return ((rows || []) as WeeklyRevenueReport[])
      .filter((r) => {
        const sid = (r as any).staffId || (r as any).staff_id || ''
        return sid === staffId && ((r as any).status) !== 'init'
      })
      .sort((a, b) => {
