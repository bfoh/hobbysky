...
function QRPanel() {
  ...
  const handlePrint = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Hobbysky Guest House — Clock-In QR</title>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-4 py-2 max-h-[60vh] overflow-y-auto pr-1">
            ...
            <div className="grid gap-2">
              ...
              <Input value={form.skills} onChange={e => setForm(f => ({ ...f, skills: e.target.value }))} placeholder="e.g. Customer service, MS Office" />
            </div>
            <div className="grid gap-2">
        ...
        {splits.map((s, i) => {
          ...
          return entry ? (
            <span key={i} className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium ${entry.className}`}>
              {entry.label} {formatGHS(s.amount)}
            </span>
          ) : null
        })}
      </div>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {(weekResult.orphanCharges ?? []).map((c) => (
                            <TableRow key={c.id}>
                              <TableCell className="text-xs"><span className="mr-1">{CATEGORY_ICONS_HR[c.category] || '📦'}</span>{c.description}</TableCell>
                              <TableCell className="text-xs text-muted-foreground">{CHARGE_CATEGORIES[c.category as keyof typeof CHARGE_CATEGORIES] || c.category}</TableCell>
                              <TableCell className="text-xs text-right">{c.quantity}</TableCell>
                              <TableCell className="text-xs text-right font-medium text-amber-700">{formatGHS(c.amount)}</TableCell>
                              <TableCell className="text-xs">
                                {c.paymentMethod === 'cash' ? 'Cash' : c.paymentMethod === 'mobile_money' ? 'MoMo' : c.paymentMethod === 'card' ? 'Card' : '—'}
                              </TableCell>
