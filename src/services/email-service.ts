...
export async function sendStaffWelcomeEmail(params: StaffWelcomeEmailParams): Promise<EmailResult> {
  ...
  const htmlContent = generateEmailHtml({
    ...
    content: `
      ...
      <p>You have been added to the Hobbysky Guest House Hotel Management System as a <strong>${role}</strong>. Below are your login credentials to access the staff portal.</p>
  ...
  const payload: EmailPayload = {
    ...
    html: htmlContent,
    text: `
Welcome to Hobbysky Guest House Staff Portal

Hi ${name},

You have been added to the Hobbysky Guest House Hotel Management System as a ${role}.
...
© ${new Date().getFullYear()} Hobbysky Guest House Hotel Management System
    `.trim()
  }
