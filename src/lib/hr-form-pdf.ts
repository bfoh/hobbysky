...
// ─── Brand colors ─────────────────────────────────────────────────────────────
const FOREST  = [30, 61, 34] as const         // #1E3D22  (resort green)
const GOLD    = [201, 150, 60] as const        // #C9963C  (gold accent)
const LIGHT_GREEN = [237, 244, 238] as const   // table header bg
...
export async function generateEmploymentApplicationPDF(): Promise<void> {
  ...
  // FORM TITLE
  // ══════════════════════════════════════════════════════════════════════════
  setColor(doc, LIGHT_GREEN, 'fill')
  doc.roundedRect(M, y, CONTENT_W, 12, 2, 2, 'F')
  setColor(doc, FOREST, 'text')
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(13)
  ...
  // ══════════════════════════════════════════════════════════════════════════
  if (y > PAGE_H - 120) { doc.addPage(); y = 20 }
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFontSize(6.5)
    setColor(doc, GRAY, 'text')
    doc.text('Hobbysky Guest House  |  Abuakwa-Manhyia, Kumasi, Ghana  |  +233 240 204 079  |  reservations@hobbyskyguesthouse.com  |  hobbyskyguesthouse.com', PAGE_W / 2, PAGE_H - 9, { align: 'center' })
    doc.text(`Confidential — For Internal Use Only    |    Page ${p} of ${totalPages}`, PAGE_W / 2, PAGE_H - 5, { align: 'center' })
  }

  doc.save('Hobbysky-Employment-Application-Form.pdf')
}
