...
// Fallback presentation data (images, features, descriptions) — prices come from the DB
const fallbackRooms = [
  {
    name: 'Standard Room',
    price: 150,
...
export default function OurRoomsSection() {
  ...
  useEffect(() => {
    async function loadLiveRooms() {
      try {
        const types = await (blink.db as any).roomTypes.list({
          orderBy: { column: 'createdAt', ascending: true },
        });

        if (types && types.length > 0) {
          const presentationRooms = types.slice(0, 3).map((dbRoom: RoomType, index: number) => {
            // Match by name similarity, or fall back to positional entry
            const fallbackMatch =
              fallbackRooms.find(
                (fb) =>
                  fb.name.toLowerCase().includes(dbRoom.name.toLowerCase()) ||
                  dbRoom.name.toLowerCase().includes(fb.name.toLowerCase())
              ) || fallbackRooms[index % fallbackRooms.length];

            return {
              id: dbRoom.id,
              name: dbRoom.name,
              price: dbRoom.basePrice,          // live price from management app
              image: fallbackMatch?.image || '/room-standard.jpg',
              features: fallbackMatch?.features || ['Comfortable Bed', 'Free Wi-Fi'],
              description: fallbackMatch?.description || 'A comfortable room for your stay.',
            };
          });
        ...
        console.error('Failed to load live room prices for Book Now page:', error);
        // Keep fallback rooms on error
      } finally {
        setLoading(false);
      }
    }
    ...
    <section className="py-20 px-4 bg-gradient-to-br from-resort-green-800 to-resort-green-950">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-14">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-resort-gold-400 mb-4">Our Rooms</h2>
          <p className="text-white/70 max-w-2xl mx-auto text-sm">
            Choose from our carefully designed rooms, each offering a unique blend of comfort, luxury, and stunning
            views of the Volta Lake.
          </p>
        </div>

        {loading ? (
          <div className="flex justify-center items-center py-20">
            <div className="w-8 h-8 border-4 border-resort-gold-500 border-t-transparent rounded-full animate-spin" />
          </div>
        ) : (
